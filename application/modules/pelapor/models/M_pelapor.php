...
class M_pelapor extends CI_Model
{
  ...
  {
    return $this->db->query("SELECT `tb_user`.`foto` FROM `tb_user` WHERE `tb_user`.`id` = '$id'")->result_array();
  }

  function getHistoryPelapor($id_user)
  {
    return $this->db->query("SELECT `pesan`.`id`, LEFT(`pesan`.`pesan_masuk`, 10) as `inbox`, `pesan`.`tgl_masuk`, `tb_user`.`id` as `user_id`, `tb_user`.`nama`, `tb_user`.`email` FROM `pesan` INNER JOIN `tb_user` ON `pesan`.`id_user` = `tb_user`.`id` WHERE `pesan`.`id_user` = '$id_user'")->row_array();
  }

  function countAllPesanByIdUser($id_user)
  {
    return $this->db->query("SELECT `pesan`.`id` FROM `pesan` WHERE `pesan`.`email` = '$id_user'")->num_rows();
  }
}
